export default function FrontPageCard({FrontPageData}) {
    return(
        <div className="flex-none max-w-sm rounded-2xl bg-white drop-shadow-lg my-2 mx-2">
        <a target="_blank" href={FrontPageData?.cta?.link}>
          <div className="relative">
            <img
              className="h-48 w-92 object-cover rounded-2xl"
              src={
                "https://media-assets.swiggy.com/swiggy/image/upload/" +
                FrontPageData?.info?.mediaFiles?.[0].url
              }
              alt=""
            ></img>
          ...
          <div className="relative h-50 mx-2 mb-4">
            ...
            <h1 className="absolute top-27 left-0 right-0 bg-[#1BA672] rounded-2xl font-bold py-2 px-3 text-white">
              {FrontPageData?.info?.offerInfoV2.otherOffers.offers?.[0]?.header}{" "}
              on pre-booking
            </h1>
